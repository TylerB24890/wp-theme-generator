...
class Create_Theme {
	...
	*/
	private function validate_form_submission(&$data) {
		...
		if(is_array($data) && !empty($data)) {
			...
			foreach($data as $k => $v) {
				...
				if($k !== 'theme_author' && $k !== 'email') {
					...
					if(strlen($v) < 1 && $k !== 'theme_description') {
						...
						if($v === 'theme_slug' || $v === 'theme_prefix') {
							// If we're on the theme_slug or prefix, make lowercase and replace spaces with dashes
							$regex = ($v === 'theme_slug' ? "/[^a-z-]/i" : "/[^a-z_]/i");
							$v = str_replace(' ', '-', strtolower($v));
							$v = preg_replace($regex, "", $v);
						}

						if($k === 'theme_description' && strlen($k) < 1) {
							$data['theme_description'] = $data['theme_name'] . ' WordPress Theme - Generated with love by Lexi -- The Elexicon Theme Generator.';
						}
			...
			$data['theme_file_name'] = strtolower(str_replace(' ', '-', $data['theme_slug']));
	...
	*/
	private function swap_theme_data($data, $dir) {
		...
		$exclude = array(
			...
			$dir . DIRECTORY_SEPARATOR . 'styles',
		...
		$paths = new RecursiveIteratorIterator(new RecursiveCallbackFilterIterator(
		new RecursiveDirectoryIterator($dir, RecursiveDirectoryIterator::SKIP_DOTS), $filter));
		...
		self::change_file_names($data, $dir);
	...
	*/
	private static function change_file_names($data, $dir) {
		...
		$file_dirs = array(
			$dir . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'js',
		...
		foreach($file_dirs as $file_dir) {
			...
			if ($handle = opendir($file_dir)) {
				// While the filename in the directory does not = the directory name
	...
	*/
	private function create_theme_zip($dest, $data) {
		...
		// If the zip was created successfully
		if($res === TRUE) {
			// Add the theme directory content to the zip file
