...
class PostType {
	...
	public function register_post_type() {
		...
		$default_labels = array(
			'name' => $plural,
			'singular_name' => $name,
			'add_new' => sprintf(__('Add New %s', Helper::$theme_slug), $name),
			'add_new_item' => sprintf(__('Add New %s', Helper::$theme_slug), $name),
			'edit_item' => sprintf(__('Edit %s', Helper::$theme_slug), $name),
			'new_item' => sprintf(__('New %s', Helper::$theme_slug), $name),
			'all_items' => sprintf(__('All %s', Helper::$theme_slug), $plural),
			'view_item' => sprintf(__('View %s', Helper::$theme_slug), $name),
			'search_items' => sprintf(__('Search %s', Helper::$theme_slug), $plural),
			'not_found' => sprintf(__('No %s found', Helper::$theme_slug), $plural),
			'not_found_in_trash' => sprintf(__('No %s found in Trash', Helper::$theme_slug), $plural),
			'parent_item_colon' => '',
			'menu_name' => $plural
		);
